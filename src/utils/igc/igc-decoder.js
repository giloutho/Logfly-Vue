import IGCParser from 'igc-parser';     // https://github.com/Turbo87/igc-parser
import smooth from 'array-smooth'
import {distance} from '@/utils/geo/trigo.js'
...
export function igcDecoding(strIgc) { 
    ...
    if (strIgc.substring(0, 1) === 'A') {
        ...
        if (strIgc.charAt(0) === "\u0013")  {
            let cleanString = strIgc.substring(1)
            strIgc = cleanString
        }
    ...
    parse(extracalculations = false, filter = false){
        ...
        try {
            ...
            if (this.fixes.length > 2) {
                ...
                this.info.offsetUTC = computeOffsetUTC(this.fixes[0].latitude, this.fixes[0].longitude, this.fixes[1].timestamp)
  ...
  analyzeFixes() {
    ...
    for (let i=1; i<this.fixes.length; i++){
        ...
        if (duration > 999) {      // duration is in milliseconds
            ...
            flTime += t				
            let d = distance(this.fixes[i].latitude, this.fixes[i].longitude, this.fixes[i-1].latitude, this.fixes[i-1].longitude, "K") * 1000;        
            if (isNaN(d)) d = 0
